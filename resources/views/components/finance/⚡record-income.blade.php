...
<div class="mx-auto max-w-lg">
    <div class="flex items-center gap-3">
        <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-green-50 text-green-600">
            <x-icon name="trend-up" class="h-5 w-5" />
        </span>
        <div>
            <h1 class="text-xl font-semibold text-slate-900">Record income</h1>
            <p class="text-sm text-slate-500">Money that came into one of your accounts.</p>
        </div>
    </div>

    @if ($this->accounts->isEmpty() || $this->categories->isEmpty())
        <x-ui.empty-state icon="wallet" title="A bit more setup needed" description="You need at least one financial account and one income category before recording income." class="mt-6">
            <x-slot:actions>
        ...
        <form wire:submit="save" class="mt-6 space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
            ...
            <div>
                ...
                <div class="relative mt-1">
                    ...
                    <input wire:model="amount" type="text" inputmode="decimal" placeholder="5,000.00" class="block w-full rounded-lg border-slate-300 pl-11 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                </div>
